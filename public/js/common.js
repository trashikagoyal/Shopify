...
document.getElementById('filterForm').addEventListener('submit', function() {
    ...
    let selectedCategories = Array.from(categoriesSelect.selectedOptions).map(option => option.value);
    let categoriesString = selectedCategories.join(',');

    let form = document.getElementById('filterForm');
    form.action = form.action.split('?')[0] + '?categories=' + encodeURIComponent(categoriesString);
});
